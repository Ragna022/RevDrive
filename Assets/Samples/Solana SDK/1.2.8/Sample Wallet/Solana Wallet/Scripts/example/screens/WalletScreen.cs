...
namespace Solana.Unity.SDK.Example
{
    public class WalletScreen : SimpleScreen
    {
        ...
        [SerializeField]
        private TextMeshProUGUI usdValue; // Dollar conversion text
        [SerializeField]
        ...
        private double _currentSolBalance = 0;
        private double _currentSolPrice = 0;
        private bool _isFetchingPrice = false;

        public void Start()
        {
            refreshBtn.onClick.AddListener(RefreshWallet);
            sendBtn.onClick.AddListener(() => { TransitionToTransfer(); });
            signBtn.onClick.AddListener(() => { manager.ShowScreen(this, "sign_screen"); });
            receiveBtn.onClick.AddListener(() => { manager.ShowScreen(this, "receive_screen"); });
            swapBtn.onClick.AddListener(() => { manager.ShowScreen(this, "swap_screen_ag"); });
            ...
            StartAutomaticPriceUpdates().Forget();
        }
        ...
        private void OnEnable()
        {
            ...
            // Update USD value when screen is enabled
            UpdateUsdValue();
        }

        private async void OnBalanceChange(double sol)
        {
            _currentSolBalance = sol;
            ...
            // Update USD value automatically
            UpdateUsdValue();
            
            GetOwnedTokenAccounts().AsAsyncUnitUniTask().Forget();
        }

        private async UniTask StartAutomaticPriceUpdates()
        {
            while (!_stopTask.IsCancellationRequested)
            {
                try
                {
                    await FetchSolPrice();
                    await UniTask.Delay(30000); // Update price every 30 seconds
                }
                catch (System.Exception ex)
                {
                    Debug.LogWarning($"Price update failed: {ex.Message}");
                    await UniTask.Delay(10000); // Wait 10 seconds before retry
                }
            }
        }

        private async UniTask FetchSolPrice()
        {
            if (_isFetchingPrice) return;
            
            _isFetchingPrice = true;
            try
            {
                using (var www = new UnityEngine.Networking.UnityWebRequest("https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd"))
                {
                    ...
                    {
                        var response = JsonUtility.FromJson<SolPriceResponse>(www.downloadHandler.text);
                        _currentSolPrice = response.solana.usd;
                        Debug.Log($"SOL price updated: ${_currentSolPrice}");
                        
                        // Update USD display with new price
                        UpdateUsdValue();
                    }
                    else
                    {
                        Debug.LogWarning("Failed to fetch SOL price from API");
                    }
                }
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"Error fetching SOL price: {ex.Message}");
            }
            finally
            {
        ...
        {
            if (_currentSolPrice > 0 && _currentSolBalance > 0)
            {
                double usdAmount = _currentSolBalance * _currentSolPrice;
                MainThreadDispatcher.Instance().Enqueue(() =>
                {
                    usdValue.text = $"${usdAmount:F2}";
                });
            }
            else
            {
        ...
        [System.Serializable]
        private class SolPriceResponse
        {
            public SolanaData solana;
        }

        [System.Serializable]
        private class SolanaData
        {
            public double usd;
        }
        ...
        public override void ShowScreen(object data = null)
        {
            ...
            // Fetch latest price when screen is shown
            FetchSolPrice().Forget();
        }
