...
public class PrometeoCarController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        currentVelocity = GetComponent<Rigidbody>().linearVelocity.magnitude ;
        Gforce = (currentVelocity - lastFrameVelocity) / (Time.deltaTime * Physics.gravity.magnitude);
        lastFrameVelocity = currentVelocity;

        // NEW CODE: Apply additional turbo force if active and applying some throttle
        if (isTurboActive && throttleAxis > 0.1f) // Only apply if turbo is active and user is pressing throttle
        {
             // Apply force in the car's forward direction
             // Multiply by throttleAxis so the force scales with how much throttle is applied
             carRigidbody.AddForce(transform.forward * turboForce * throttleAxis, ForceMode.Force);
    ...
    public void GoForward(){
      ...
      // Determine the current torque multiplier (default or turbo)
      float currentTorqueMultiplier = accelerationMultiplier * 50f;
      if (isTurboActive)
      {
          currentTorqueMultiplier *= turboMultiplier;
          // Keep or remove debug as needed:
          Debug.Log("Applying Turbo Torque: " + (currentTorqueMultiplier * throttleAxis) + " (Turbo Active)");
      }
      else // Keep or remove debug as needed:
      {
        Debug.Log("Applying Standard Torque: " + (currentTorqueMultiplier * throttleAxis) + " (Standard)");
      }


      //If the car is going backwards, then apply brakes...
      if(localVelocityZ < -1f){
        Brakes();
      }else{
        // MODIFIED LINE: Only check maxSpeed if turbo is NOT active
        if(isTurboActive || Mathf.RoundToInt(carSpeed) < maxSpeed){
            //Apply positive torque...
            frontLeftCollider.brakeTorque = 0;
            frontLeftCollider.motorTorque = currentTorqueMultiplier * throttleAxis;
            frontRightCollider.brakeTorque = 0;
            frontRightCollider.motorTorque = currentTorqueMultiplier * throttleAxis;
            rearLeftCollider.brakeTorque = 0;
            rearLeftCollider.motorTorque = currentTorqueMultiplier * throttleAxis;
            rearRightCollider.brakeTorque = 0;
            rearRightCollider.motorTorque = currentTorqueMultiplier * throttleAxis;
        }else {
            // If the maxSpeed has been reached and turbo is NOT active, then stop applying torque.
    ...
    public void GoReverse(){
      ...
      // Determine the current torque multiplier (default or turbo - though turbo usually applies forward)
      // We'll apply the same multiplier for reverse for simplicity, but you might want to exclude it.
      float currentTorqueMultiplier = accelerationMultiplier * 50f;
       if (isTurboActive)
       {
           currentTorqueMultiplier *= turboMultiplier; // Consider if turbo should affect reverse
       }
    ...
    // This function is used to recover the traction of the car when the user has stopped using the car's handbrake.
     ...
     {
         isTurboActive = false;
         cooldownTimer = turboCooldown; // Start the cooldown timer
